describe('Create vacancy Test',() => {
    it ('Create vacancy test',() => {
        cy.fixture('empNeeds').then(data => {
            ...
            cy.log('Прописываем обязанность')
            cy.get('.desktop-modal__content > .vacancy-need-wrapper > .form > :nth-child(1) > .form__labels > .labels > :nth-child(3) > .form-control > .form-area').type(data.duty, { force: true })
            cy.log('Прописываем Требования')
            cy.get('.desktop-modal__content > .vacancy-need-wrapper > .form > :nth-child(1) > .form__labels > .labels > :nth-child(4) > .form-control > .form-area').type(data.requirements, { force: true })
